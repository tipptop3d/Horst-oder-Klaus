...
class Plotter:
    def __init__(self, current_x=X_LEFT_BOUND,
                 current_y=Y_UPPER_BOUND, lifted=False):
    ...
    def move_to(self, x=None, y=None, wait=True, x_wait=False):
        ...
        if not (X_LEFT_BOUND <= x <= X_RIGHT_BOUND
                or Y_LOWER_BOUND <= y <= Y_UPPER_BOUND):
    ...
    def draw(self, expression):
        ...
        while self.current_x < X_RIGHT_BOUND:
            ...
            if abs(y_angle_speed) > Y_MAX_ANGLE_SPEED:
                ...
                # dividing speed_factor with x-speed
                # gives the lowered x-speed, retaining ratio
